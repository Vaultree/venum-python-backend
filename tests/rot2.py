...
def test_scheme_rotation(input):
    ...
    for pot in root_list:
        for rot in range(n):
            ...
            c1 = rotate_polynomial_cryptogram(c0, n)
            #print("C1: ", c1)
    ...
    t = rotate_polynomial([1,2,3,4,5,6,7,8],8)
    print("Resultado: ", t)
...
# -----------------------------------------------------------------------
def rotate_polynomial_cryptogram(c0: Cryptogram, n):

    poly1 = c0.body
    poly2 = c0.mask

    poly1 = rotate_polynomial(poly1, n)
    poly2 = rotate_polynomial(poly2, n) 
    
    c1 = Cryptogram(body=poly1, mask=poly2, batched=c0.batched, q=c0.q)
